...
Gaji uses a **microservices architecture (Pattern B: API Gateway)** with **multi-repository structure** managed via Docker Compose.
...
git clone --recurse-submodules https://github.com/SideProjectSFY/gaji.git
cd gaji

# 2. Setup environment
cp .env.example .env
# Edit .env with your credentials (DB_PASSWORD, GEMINI_API_KEY)
...
git submodule update --remote gajiBE
```
...
cd ..
git add gajiBE
git commit -m "chore: update gajiBE submodule"
git push
```

### Hot Reload (Development Mode)

- **Spring Boot**: Java files auto-reload (Spring DevTools)
- **FastAPI**: Python files auto-reload (uvicorn --reload)
- **Vue.js**: Vue files auto-reload (Vite HMR, <2s)
